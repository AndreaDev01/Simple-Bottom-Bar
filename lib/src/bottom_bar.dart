    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: Row(
          ...
          children:
              widgets.map((menuItem) {
                return GestureDetector(
                  onTap: () {
                    onChange(menuItem.index);
                    setState(() {
                  ...
                  child: AnimatedSwitcher(
                    ...
                    child:
                        menuItem.index == selected
                            ? AnimatedContainer(
                              key: ValueKey("selected_${menuItem.index}"),
                              duration: Duration(milliseconds: 300),
                              ...
                              child: Row(
                                ...
                                children: [
                                  Icon(
                                    menuItem.icon,
                                    size: 25,
                                    color: bottomBarConfig.activeIconColor,
                                  ),
                                  SizedBox(width: 15),
                                  Text(
                                    menuItem.text,
                                    style: bottomBarConfig.textStyle,
                                  ),
                                ],
                              ),
                            )
                            : AnimatedContainer(
                              key: ValueKey("unselected_${menuItem.index}"),
                              duration: Duration(milliseconds: 300),
                              padding: EdgeInsets.all(10),
                              child: Icon(
                                menuItem.icon,
                                size: 30,
                                color: bottomBarConfig.iconsColor,
                              ),
                            ),
                  ),
                );
